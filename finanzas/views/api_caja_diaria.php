...
while($fila = $ingresos_result->fetch_assoc()){ $ingresos_por_metodo[$fila['metodo_pago']] = $fila['total']; $total_ingresos += $fila['total']; }
$stmt->close();

// Egresos
$sql_egresos = "SELECT e.forma_pago, SUM(e.monto) as total FROM egresos e JOIN recibos r ON e.recibo_id = r.id JOIN asesor a ON r.id_asesor = a.id_asesor WHERE e.fecha = ? AND a.id_sede = ? GROUP BY e.forma_pago";
...
while($fila = $egresos_result->fetch_assoc()){ $egresos_por_metodo[$fila['forma_pago']] = $fila['total']; $total_egresos += $fila['total']; }
$stmt->close();
...
while($fila = $gastos_result->fetch_assoc()){ $gastos_por_metodo[$fila['metodo_pago']] = $fila['total']; $total_gastos += $fila['total']; }
$stmt->close();
